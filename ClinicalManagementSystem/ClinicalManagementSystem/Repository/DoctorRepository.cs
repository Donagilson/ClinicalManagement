...
namespace ClinicalManagementSystem2025.Repository
{
    public class DoctorRepository : IDoctorRepository
    {
        ...
        public async Task<int> AddDoctorAsync(Doctor doctor)
        {
            using var connection = new SqlConnection(_connectionString);
            const string sql = @"
                INSERT INTO TblDoctors (UserId, Specialization, SpecializationId, Qualification, Experience,
                                      DepartmentId, ConsultationFee, AvailableFrom, AvailableTo, IsActive)
                OUTPUT INSERTED.DoctorId
                VALUES (@UserId, @Specialization, @SpecializationId, @Qualification, @Experience,
                        @DepartmentId, @ConsultationFee, @AvailableFrom, @AvailableTo, @IsActive)";
        ...
        public async Task<IEnumerable<Doctor>> GetAllDoctorsWithDetailsAsync()
        {
            ...
            const string sql = @"
                SELECT d.DoctorId, d.UserId, d.Specialization, d.SpecializationId, d.Qualification,
                ...
                INNER JOIN TblUsers u ON d.UserId = u.UserId
                INNER JOIN TblDepartments dep ON d.DepartmentId = dep.DepartmentId
                LEFT JOIN TblSpecializations s ON d.SpecializationId = s.SpecializationId
                ORDER BY u.FullName";
            ...
            while (await reader.ReadAsync())
            {
                ...
                var specializationIdOrdinal = reader.GetOrdinal("SpecializationId");
                ...
                doctors.Add(new Doctor
                {
                    ...
                    Specialization = reader.GetString("Specialization"),
                    SpecializationId = reader.IsDBNull(specializationIdOrdinal) ? null : reader.GetInt32(specializationIdOrdinal),
        ...
        public async Task<IEnumerable<Specialization>> GetAllSpecializationsAsync()
        {
            ...
            const string sql = @"
                SELECT s.SpecializationId, s.SpecializationName, s.Description,
                       s.DepartmentId, d.DepartmentName, s.IsActive, s.CreatedDate
                FROM TblSpecializations s
                INNER JOIN TblDepartments d ON s.DepartmentId = d.DepartmentId
                WHERE s.IsActive = 1
                ORDER BY d.DepartmentName, s.SpecializationName";
        ...
        public async Task<IEnumerable<Specialization>> GetSpecializationsByDepartmentAsync(int departmentId)
        {
            ...
            const string sql = @"
                SELECT s.SpecializationId, s.SpecializationName, s.Description,
                       s.DepartmentId, d.DepartmentName, s.IsActive
                FROM TblSpecializations s
                INNER JOIN TblDepartments d ON s.DepartmentId = d.DepartmentId
